...
Practicing my self-taught studies of C# by porting the 6 basic Python projects presented on the YouTube channel 'freeCodeCamp Español' from Python to .NET C#.

The code for each of the 6 mini applications will be shown and explained below.
...
## 1 Mad Libs
The system will ask for some words and then concatenate the words entered by the user into fun sentences.
...
## 2 Guess the number
The user guesses the random number generated by the computer
...
## 3 The computer guess the number 
The computer must guess the number selected by the player
...
Playing the classic game Rock-Paper-Scissors with the computer
...
## 5 Hangman game
The classic Hangman game that you have to guess a word with a limited attempts. You have the download the files Palabras.cs and Ahorcado_diagramas.cs in the same folder in order to run the code below.
...
Search algorithm to find the position of a target value within a sorted array. This algorithm is fastest that the traditional way of compare the elements in the array one by one. Mention that the Python implementation of this code is much slower (expected since Python is an interpreted programming language).
